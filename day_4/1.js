...
Object.values(bingoCards).forEach(({ rows }, cardIndex) => {
    ...
    for (const [ballIndex, ball] of bingoBalls.entries()) {
        ...
        if (bingo) {
            ...
            if (cardWithQuickestWin === null || bingoCards[cardWithQuickestWin].turnsToWin > bingoCards[cardIndex].turnsToWin) {
                cardWithQuickestWin = cardIndex;
            }
            break;
        }
    }
...
const winningCard = bingoCards[cardWithQuickestWin];
